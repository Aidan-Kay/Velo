 ...
 * Returns an updated array of photo paths (local file:// URLs for downloaded photos,
 ...
 */
export async function downloadItemPhotos(itemId: string, photos: string[], forceRedownload = false): Promise<string[]> {
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    try {
      ...
      if (!forceRedownload && fs.existsSync(filePath)) {
        updatedPhotos.push(`file://${filePath.replace(/\\/g, "/")}`);
